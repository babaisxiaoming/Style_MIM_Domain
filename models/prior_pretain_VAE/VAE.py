...
class Down(torch.nn.Module):
    def __init__(self, in_ch, out_ch, norm_type=2, kernal_size=2, stride=2, soft=False):
        super().__init__()
        self.conv = torch.nn.Sequential(
...
class VAE(torch.nn.Module):
    ...
    def __init__(self, n_channels=1, n_class=1, norm_type=2, n_fmaps=[8, 16, 32, 64, 128, 256], dim=128, soft=False):
        super().__init__()
        # self.SConv2d = SConv2d(1,n_fmaps[0],3,padding=1,bias=True)
        ...
        self.fc_mean = torch.nn.Linear(12544, dim)
        self.fc_std = torch.nn.Linear(12544, dim)
        self.fc2 = torch.nn.Linear(dim, 12544)
    ...
    def forward(self, x, if_random=False, scale=1, mid_input=False, dropout=0.0):
        ...
        if not mid_input:
            ...
            # input_x = self.SConv2d(input_x)
            x = self.in_block(x)  # 1x8x224
            x = self.down1(x)  # 1x16x112
            x = self.down2(x)  # 1x32x56
            x = self.down3(x)  # 1x64x28x28
            x = self.down4(x)  # 1x128x14x14
            x = self.down5(x)  # 1x256x7x7
            x = x.view(x.size(0), 12544)
            x_mean = self.fc_mean(x)  # 1x128
            x_std = nn.ReLU()(self.fc_std(x))  # 1x128
            # data_dict['mean'] = x_mean
            # data_dict['std'] = x_std
            z = torch.randn(x_mean.size(0), x_mean.size(1)).type(torch.cuda.FloatTensor)  # 1x128
            if if_random:
        ...
        x = self.up1(x)  # 1x128x14x14
        if dropout: x = torch.nn.functional.dropout(x, p=dropout, training=True)
        x = self.up2(x)  # 1x64x28x28
        if dropout: x = torch.nn.functional.dropout(x, p=dropout, training=True)
        x = self.up3(x)  # 1x32x64x64
        if dropout: x = torch.nn.functional.dropout(x, p=dropout, training=True)
        x = self.up4(x)  # 1x16x112x112
        if dropout: x = torch.nn.functional.dropout(x, p=dropout, training=True)
        x = self.up5(x)  # 1x8x224x224
        if dropout: x = torch.nn.functional.dropout(x, p=dropout, training=True)
        x = self.out_block(x)
        x = self.final(x)
        ...
        if not mid_input:
            return self.klloss(x_mean, x_std), x
        else:
            return x
...
if __name__ == '__main__':
    ...
    model = VAE(n_channels=1, n_class=1, norm_type=1, dim=128)
    y_hat = model(x)
