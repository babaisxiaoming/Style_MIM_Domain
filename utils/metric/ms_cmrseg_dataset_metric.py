...
def metrics(img_gt, img_pred, modality, apply_hd=False, apply_asd=False, class_name=("myo", "lv", "rv")):
    """
    ...
    for c, cls_name in zip(np.array(range(len(class_name))) + 1, class_name):
        ...
        # Compute the Dice
        dice = dc(gt_c_i, pred_c_i) # 1x192x192 1x192x192 one_hot编码 # 调用的函数
        ...
        if apply_hd or apply_asd:
            if np.sum(gt_c_i) == 0:
                ...
                h_d = -1
                a_sd = -1
                print("Prediction empty for {}".format(modality))
            else:
