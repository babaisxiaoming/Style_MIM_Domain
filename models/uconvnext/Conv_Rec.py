...
class Seg_Deep(nn.Module):
    ...
    def seg_loss(self, label, seg):
        ...
        if self.scale != 1:
            down_label = F.interpolate(label.to(seg.dtype), scale_factor=1 / self.scale, mode='bilinear')
        else:
            down_label = label

        down_loss = self.train_loss(seg, down_label.squeeze(1).long())
        return down_loss
...
class Rec_Decoder(nn.Module):
    def __init__(self,
        ...
        self.auxility = UConvNeXt.Decoder()
    ...
    def forward(self, image, label, encoder_feats, decoder_feats):
        # 分割的深度监督
...
if __name__ == '__main__':
    image = torch.randn(1, 3, 224, 224).to(torch.float32).cuda()
    label = torch.randint(0, 1, (1, 224, 224)).cuda()

    encoder_1 = torch.randn(1, 96, 56, 56).cuda()
    encoder_2 = torch.randn(1, 192, 28, 28).cuda()
    encoder_3 = torch.randn(1, 384, 14, 14).cuda()
    encoder_4 = torch.randn(1, 769, 7, 7).cuda()
    # 主干网络 decoder 的feats
    decoder_1 = torch.randn(1, 192, 28, 28).cuda()
    decoder_2 = torch.randn(1, 96, 56, 56).cuda()
    decoder_3 = torch.randn(1, 4, 224, 224).cuda()

    model = Rec_Decoder().cuda()
    y_hat = model(image, label, [encoder_1, encoder_2, encoder_3, encoder_4], [decoder_1, decoder_2, decoder_3])
    print(y_hat)