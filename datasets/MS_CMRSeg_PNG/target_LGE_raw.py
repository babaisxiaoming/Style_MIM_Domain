...
class LGE_SET(Dataset):
    def __init__(self, dir):
        ...
        self.labs = np.zeros((1, 1, 224, 224))
        self.names = []
        ...
        for pat_id in range(6, 46):
            ...
            # name
            pat_name = f'pat_{pat_id}_lge'
            for i in range(nimg.shape[-1]):
                self.names.append(f'{pat_name}_{i}')
    ...
    def __getitem__(self, item):
        ...
        return torch.from_numpy(npimg).type(dtype=torch.FloatTensor), \
               torch.from_numpy(nplab).type(dtype=torch.LongTensor), name
